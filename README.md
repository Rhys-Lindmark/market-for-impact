...
- A dedicated `/san-francisco` major-donor brief that turns the local research into a decision funnel: 6,688 filing-address EINs, 548 city-contractor names, six alphabetical diligence records, and zero recommendation-ready opportunities. It names the five evidence gates required before a $100,000, $1 million, or $10 million gift without inventing a ranking or impact conversion.
...
The dedicated donor brief at `/san-francisco` is the flagship entry point for this work. Its six organizations are alphabetical, not ranked, and the current recommendation-ready count is zero. Every candidate remains blocked pending a verified legal and donation entity, program-level outcome evidence, a defensible causal case, a written marginal plan at relevant gift sizes, and time-bound follow-up milestones.
...
The first broad-cause market uses Charity Navigator’s live `LGBTQ rights` search as a discovery input. The snapshot paginates the complete current search universe and preserves EIN, headquarters, score, stars, advisory state, and source tags for every row. It fetches a fixed first-ten profile-review cohort for completed-beacon and profile-publication fields; the rest remain visibly source-index-only rather than silently inheriting missing detail.

The explorer supports local search, headquarters-state and star filters, and 12-row pagination. Cross-source reconciliation uses Unicode/whitespace/case-normalized exact names only. Charity Navigator EINs remain authoritative for its own profiles, but accepted MFI ledgers generally do not expose recipient EINs, so no cross-source identity is mislabeled as EIN-verified.
