  ...
  return <div className="sf-home givebetter">
    ...
    <main>
      ...
      <section aria-label="Four current charity picks">{picks.map((pick, i) => <article className="sf-home-charity" id={pick.slug} key={pick.slug}>
        <figure>{photos[pick.slug] ? <><img src={`${root}${photos[pick.slug].src}`} alt={photos[pick.slug].caption} width="480" height="480" loading="lazy" /><figcaption><a href={photos[pick.slug].source}>{photos[pick.slug].caption}</a></figcaption></> : <div className="sf-home-research-note"><h3>{pick.name}</h3><p>Program-specific research</p><p>This estimate concerns the clinical pathway described here, not an unrestricted organization-wide gift.</p></div>}</figure>
