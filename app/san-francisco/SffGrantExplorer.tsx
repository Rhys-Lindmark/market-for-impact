...
type Partner = {
  ...
  currentDiligenceReview: null | {
    reviewStatus: string;
    reviewStatusLabel: string;
    intervention: { type: string; theoryOfChange: string; causalBoundary: string };
    nativeSignals: { label: string; value: string; period: string; signalType: string; limitation: string }[];
    financials: { status: string; boundary: string };
    evidenceLayers: { publisher: string; title: string; url: string; design: string; finding: string; transferLimit: string }[];
    fundingRoom: { display: string; boundary: string };
        ...
        {data.partners.map((partner) => (
          <article key={partner.id}>
            ...
            <dl>
              ...
              <div><dt>Candidate diligence screen</dt><dd>{partner.currentDiligenceReview ? <span className="sf-sff-diligence-review"><b>{partner.currentDiligenceReview.reviewStatusLabel}</b><p>{partner.currentDiligenceReview.decisionSummary}</p><strong>Strongest current signals</strong>{partner.currentDiligenceReview.nativeSignals.map((signal) => <small key={signal.label}>{signal.value} · {signal.label} · {signal.period}<br />{signal.limitation}</small>)}<strong>Evidence ladder</strong>{partner.currentDiligenceReview.evidenceLayers.map((layer) => <a key={layer.url} href={layer.url} target="_blank" rel="noreferrer">{layer.publisher}: {layer.title} ↗</a>)}<strong>Financials · {partner.currentDiligenceReview.financials.status.replaceAll('-', ' ')}</strong><small>{partner.currentDiligenceReview.financials.boundary}</small><strong>Funding room · {partner.currentDiligenceReview.fundingRoom.display}</strong><small>{partner.currentDiligenceReview.fundingRoom.boundary}</small><small>{partner.currentDiligenceReview.missingForRecommendation.length} unresolved recommendation gates</small></span> : 'Not yet screened for candidate diligence'}</dd></div>
