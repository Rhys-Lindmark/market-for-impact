...
test('BVHPF report preserves SF site assumption and funding caveats',async({page})=>{
 ...
 await expect(row).toContainText('Subjective 65% SF-resident share');await expect(row.locator('td a')).toHaveAttribute('aria-label',/measured residence unavailable/);
