...
test('CalYouth separates statewide from SF cost-effectiveness',async({page})=>{
 ...
 const row=page.locator('[data-research-slug="california-coalition-for-youth"]');await expect(row).toHaveAttribute('data-estimate-geography','San Francisco');await expect(row).toContainText('(California)');
