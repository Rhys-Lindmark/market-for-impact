...
for(const [slug,title,price] of [['brightline-defense','Brightline Defense','$3.6M'],['rebuilding-together-sf','Rebuilding Together San Francisco','$5M']]){
 test(`${slug}: inspect modeled price and uncertainty`,async({page})=>{
  ...
  await expect(page.locator('.report-reading-column article')).toContainText(price);
  await expect(page.locator('.report-reading-column article')).toContainText('No finite positive price');
  await expect(page.locator('.report-reading-column article')).toContainText('not a core ranking input');
