...
test('phone donors can inspect the 6,688 to 25 San Francisco research funnel', async ({ page }, testInfo) => {
  ...
  await expect(page.locator('.sf-research-stages strong')).toHaveText(['6,688', '1,000', '100', '25', '18']);
  await expect(page.locator('.sf-deep-queue article')).toHaveCount(25);
  await expect(page.locator('.sf-deep-queue article>b').filter({ hasText: 'CEA not started' })).toHaveCount(7);
  await expect(page.locator('.sf-deep-queue article>b').filter({ hasText: 'Initial review complete' })).toHaveCount(0);
  await expect(page.locator('.sf-deep-queue article>b').filter({ hasText: 'Exploratory model' })).toHaveCount(18);
