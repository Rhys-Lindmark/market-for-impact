...
test('PHAdvocates prices local health without hiding uncertainty',async({page})=>{
 ...
 const row=page.locator('[data-research-slug="public-health-advocates"]');await expect(row).toContainText('(California)');await expect(row).toHaveAttribute('data-estimate-geography','San Francisco');
