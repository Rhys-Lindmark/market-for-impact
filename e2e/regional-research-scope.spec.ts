...
test('Bay-only list and expanded geography archive preserve all reports',async({page})=>{
 ...
 for(const [geography,count] of [['California',6],['California / Nevada',1],['U.S.',8],['International',6]] as const)await expect(page.locator('[data-geography="'+geography+'"]')).toHaveCount(count);
