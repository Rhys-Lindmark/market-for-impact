...
test('Roots preserves Bay boundary and conditional whole-gift model',async({page})=>{
 ...
 const row=page.locator('[data-research-slug="roots-community-health"]');await expect(row).toContainText('$3.6M');await expect(row).toHaveAttribute('data-estimate-geography','Bay Area');await expect(row).not.toContainText('(Bay Area)');await row.locator('a').first().click();
