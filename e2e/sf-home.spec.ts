...
for (const path of ['/', '/san-francisco']) {
  test(`SF front door ${path} has four model-priced picks and research navigation`, async ({ page }) => {
    ...
    await expect(page.locator('.sf-home-org-card h4')).toHaveText(['The ReCARES Network', 'GLIDE', 'Breathe California', 'Pacific Vision Foundation']);
    await expect(page.locator('.sf-home-charity-body section:nth-child(2) strong')).toHaveText(['$186K per better life (10 QALYs)', '$427K per better life (10 QALYs)', '$533K per better life (10 QALYs)', '$714K per better life (10 QALYs)']);
