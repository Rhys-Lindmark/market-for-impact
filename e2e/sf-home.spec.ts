...
for (const path of ['/', '/san-francisco']) {
  test(`SF front door ${path} has four model-priced picks and research navigation`, async ({ page }) => {
    ...
    await expect(page.locator('.sf-home-org')).toHaveText(['San Francisco AIDS Foundation ↗', 'Project Homeless Connect ↗', 'GLIDE ↗', 'Breathe California ↗']);
    await expect(page.locator('.sf-home-price')).toHaveText(['≈ $56.3K', '≈ $71.1K', '≈ $427K', '≈ $533K']);
