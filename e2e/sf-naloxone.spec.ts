...
test('SFAF report has priced model, residual-overlap sensitivity and clear funding boundary', async ({ page, request }) => {
  ...
  await expect(page.locator('#summary')).toContainText('$2,018,525');
  await expect(page.locator('#summary')).toContainText('whole gift');
  await expect(page.locator('#cost-effectiveness')).toContainText('Gross associated resources');
