...
for(const [slug,title] of [['pacific-vision-foundation','Pacific Vision Foundation']]){
 test(`${slug}: conditional price, signed nulls and inspectable model`,async({page})=>{
  ...
  await expect(page.locator('.report-reading-column article')).toContainText('No finite positive-health ratio');
  expect(await page.evaluate(()=>document.documentElement.scrollWidth<=innerWidth)).toBe(true);
  const [response]=await Promise.all([page.waitForResponse(r=>r.url().includes('/api/sf-surgical-access-models')),page.getByRole('link',{name:'the historical conditional model'}).click()]);
