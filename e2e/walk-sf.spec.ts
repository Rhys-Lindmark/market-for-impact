...
test('Walk SF whole-gift report and central SF ranking',async({page})=>{
 ...
 await page.goto('/research');const row=page.locator('[data-research-slug="walk-san-francisco"]');await expect(row).toContainText('$5.8M');expect(Number(await row.getAttribute('data-cost-per-ten-qalys'))).toBeCloseTo(d.evaluated[0].sfUsdPer10Qaly,2);
});
