import data from '../data/international/helen-keller-v1.json' with {type:'json'};
export {data};
export function calculate(o={}){
 ...
 if(!p.giftUsd||!p.nativeUsdPerDeath)throw new RangeError('cost');
 for(const k of ['earlyYears','olderYears'])if(!Number.isInteger(p[k])||p[k]>200)throw new RangeError(k);
 const L=(T,s,u)=>Array.from({length:T},(_,i)=>u*(s/(1+p.discountRate))**(i+.5)).reduce((a,b)=>a+b,0);
 const qPerDeath=(p.earlyShare*L(p.earlyYears,p.earlyAnnualSurvival,p.earlyUtility)+(1-p.earlyShare)*L(p.olderYears,p.olderAnnualSurvival,p.olderUtility))/(1+p.discountRate)**p.delayYears;
