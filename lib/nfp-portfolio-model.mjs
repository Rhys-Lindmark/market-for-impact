...
export const mortalityHorizonSensitivityInputs = {
  label: "baseline-discounted actuarial-anchored NFP lifetime best guess",
  ...
  discountFromPregnancyBaselineYears: 10,
  observedWindowYearsAfterAssumedDeath: 10,
  postAge20PersistencePrior: 0.50,
...
export function calculateNfpMortalityHorizonSensitivity(
  ...
  requireFinite(horizonInputs.discountFromPregnancyBaselineYears, "horizonInputs.discountFromPregnancyBaselineYears", { min: 0, max: 100 });
  requireFinite(horizonInputs.observedWindowYearsAfterAssumedDeath, "horizonInputs.observedWindowYearsAfterAssumedDeath", { min: 0, max: 100 });
  requireFinite(horizonInputs.postAge20PersistencePrior, "horizonInputs.postAge20PersistencePrior", { min: 0, max: 1 });
  const discountedAtAge10QalyPerPreventedDeath = horizonInputs.annualDiscountRate === 0
  ...
  const baselineDiscountFactor = Math.pow(1 + horizonInputs.annualDiscountRate, horizonInputs.discountFromPregnancyBaselineYears);
  const fullyCreditedObservedWindowQaly = (horizonInputs.annualDiscountRate === 0
    ? horizonInputs.healthUtility * horizonInputs.observedWindowYearsAfterAssumedDeath
    : horizonInputs.healthUtility
      * (1 - Math.pow(1 + horizonInputs.annualDiscountRate, -horizonInputs.observedWindowYearsAfterAssumedDeath))
      / horizonInputs.annualDiscountRate)
    / baselineDiscountFactor;
  const fullLifetimeBaselineQaly = discountedAtAge10QalyPerPreventedDeath
    / baselineDiscountFactor;
  const postAge20BaselineQaly = fullLifetimeBaselineQaly - fullyCreditedObservedWindowQaly;
  const discountedFiniteQalyPerPreventedDeath = fullyCreditedObservedWindowQaly
    + horizonInputs.postAge20PersistencePrior * postAge20BaselineQaly;
  ...
  return {
    ...
    discountedAtAge10QalyPerPreventedDeath,
    fullyCreditedObservedWindowQaly,
    fullLifetimeBaselineQaly,
    postAge20BaselineQaly,
...
// Publication should use this explicit lifetime judgment as its modeled best
// guess while retaining calculate() as the accepted-v3 follow-up-bound diagnostic.
export const calculatePublishedLifetimeBestGuess = calculateNfpMortalityHorizonSensitivity;
