...
// Author reports specifying priors before outputs; chronology not independently verified.
// Weights reflect substantial uncertainty,
...
export function calculate(i=inputs,ss=scenarios){
 if(i.maxModeledGiftUsd!==10000||!Number.isFinite(i.giftUsd)||i.giftUsd<0||i.giftUsd>10000)throw new RangeError('gift outside bounded 0–10000 scale');
 for(const key of ['paymentFeeUsd','totalExpenseUsd','reportedRecipientEquivalents'])if(!Number.isFinite(i[key]))throw new RangeError('finite cost inputs');
