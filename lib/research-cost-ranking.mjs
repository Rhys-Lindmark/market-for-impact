...
import {hacModel,spurModel} from './urban-policy-model.mjs';
import oa from '../data/san-francisco/oa-colonoscopy-cea-v1.json' with { type: 'json' };
import {cataractAccessModel,colonoscopyAccessModel} from './surgical-access-model.mjs';
...
export const researchCostRanking = [
  ...
  scenarioRow('spur',{organization:spur.organization,scenarios:spur.scenarios.filter(s=>spur.coreScenarioKeys.includes(s.key)).map(s=>({...s,name:s.key}))},spurModel),
  scenarioRow('operation-access',{organization:oa.organization.name,scenarios:oa.scenarios},s=>colonoscopyAccessModel(s,oa.resourceSensitivity)),
