...
import breathe from '../data/san-francisco/breathe-coverage-v2.json' with { type: 'json' };
import {calculate as breatheCoverage} from './breathe-coverage-model.mjs';
...
export const researchCostRanking = [
  ...
  scenarioRow('spur',{organization:'SPUR',scenarios:spur.scenarios.filter(s=>['central','favorable','pessimisticPositive'].includes(s.id)).map(s=>({...s,name:s.id}))},s=>({costPerTenQalys:spurPortfolioModel(s,spur.budget.totalUsd).sfUsdPer10Qaly,bayCostPerTenQalys:spurPortfolioModel(s,spur.budget.totalUsd).bayUsdPer10Qaly})),
  ...
  {...scenarioRow('clinic-by-the-bay',{organization:'Clinic by the Bay',scenarios:clinic.scenarios.map(s=>({...s,name:s.id}))},s=>({costPerTenQalys:clinicModel(clinicInputs(clinic,s)).donor_sf_per_10q})),centralUsdPerTenQalys:clinicExpected(clinic).donor_sf_per_10q,bayUsdPerTenQalys:clinicExpected(clinic).donor_bay_per_10q,estimateBasis:'Subjective signed-health expectation; unweighted central and weight sensitivity in report'},
  scenarioRow('breathe-california', {organization:'Breathe California',scenarios:breathe.scenarios.map(s=>({...s,name:s.id}))},s=>{const r=breatheCoverage(breathe,s);return {costPerTenQalys:r.sf.donor_per_10q,bayCostPerTenQalys:r.bay.donor_per_10q};}),
