...
export function researchEffortSummary(data,organization){
 ...
 if(!record?.sessions.length)return {label:'Research time: not recorded on an unrecorded AI model',recorded:false,details:data.historicalCoverage,minutes:null};
 ...
 return {
  label:'Research time: '+(display===0?'less than 0.1':String(display))+' min on '+models.join(' + ')+(record.coverage==='partial'?' (partial record)':''),
