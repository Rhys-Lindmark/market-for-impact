...
export function researchEffortSummary(data,organization,historical=null){
 ...
 if(!record?.sessions.length){
  if(older)return {label:'Research time: ~'+Math.round(historical.minutes)+' min on '+historical.model.name,recorded:false,estimated:true,minutes:historical.minutes,bullets:['Research — organization and evidence review.','Modeling — cost-effectiveness analysis.','Historical estimate — average of '+historical.samples.length+' report records; AI model assumed.']};
 ...
 return {
  label:'Research time: '+(display===0?'less than 0.1':String(display)+(record.coverage==='partial'?'+':''))+' min on '+models.join(' + '),
