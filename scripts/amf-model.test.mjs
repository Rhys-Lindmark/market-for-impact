...
test('AMF global benefits never enter SF ranking and retain donor cost',()=>{
 ...
 const index=fs.readFileSync('lib/unified-research-index.ts','utf8');assert.match(index,/International/);assert.match(index,/sfUsdPerTenQalys:null as number\|null/);
 const page=fs.readFileSync('app/research/page.tsx','utf8');assert.match(page,/SF-resident QALYs/);assert.doesNotMatch(page,/US-wide research|International research/);
});
