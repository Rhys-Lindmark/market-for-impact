...
const [source, irsUniverse, candidateUniverse, diligence] = await Promise.all([
...
const snapshot = validateSfSffGrants(buildSfSffGrants({ source, irsUniverse, candidateUniverse, diligence }));
...
console.log(`SFF community grants current: ${snapshot.summary.publishedPartnerRowCount} partner rows, $${snapshot.summary.publishedPartnerTotalFundingUsd.toLocaleString('en-US')}, ${snapshot.summary.diligenceMatchRowCount} diligence match.`);
