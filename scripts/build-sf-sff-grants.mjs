...
const [source, fiscalSponsorSource, irsUniverse, candidateUniverse, diligence] = await Promise.all([
...
const snapshot = validateSfSffGrants(buildSfSffGrants({ source, fiscalSponsorSource, irsUniverse, candidateUniverse, diligence }));
...
console.log(`SFF community grants current: ${snapshot.summary.publishedPartnerRowCount} partner rows, $${snapshot.summary.publishedPartnerTotalFundingUsd.toLocaleString('en-US')}, ${snapshot.summary.sourceReportedFiscalSponsorRowCount} exact fiscal-sponsor source matches.`);
