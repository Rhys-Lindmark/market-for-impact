...
test('policy thresholds are not reported as central cost effectiveness',()=>{
 for(const slug of ['western-center-on-law-and-poverty','worksafe']){
  ...
  assert.equal(r.model.scenarios.find(s=>s.id==='threshold-hundred-thousand').editionQalys,10);
 }
...
test('policy accounts retain consistent recipient-specific means',()=>{
 assert.equal(expenseAverage(reports.find(r=>r.slug==='western-center-on-law-and-poverty')),6918761);
