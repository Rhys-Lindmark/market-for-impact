...
test('all 30 reviews have unique, ascending central 10-QALY estimates', () => {
  assert.equal(rows.length, 30);
  assert.equal(new Set(rows.map(r => r.slug)).size, 30);
  rows.forEach((r, i) => {
  ...
  assert.deepEqual(rows.slice(0, 4).map(r => r.slug), ['san-francisco-aids-foundation', 'project-homeless-connect', 'glide', 'compass-family-services']);
