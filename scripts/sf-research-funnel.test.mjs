...
test('builds a nested research funnel without implying impact', () => {
  ...
  assert.deepEqual(snapshot.summary, { universeCount: 6688, machineEligibleCount: 1103, shallowScreenCount: 1000, priorityReviewCount: 100, deepDiveQueueCount: 25, completedInitialReviewCount: 12, exploratoryModelCount: 6, completedCostEffectivenessCount: 0 });
  ...
  assert.deepEqual(snapshot.deepDiveRows.filter((row) => row.costEffectivenessStatus === 'exploratory-model').map((row) => row.displayName), ['Curry Senior Center', 'GLIDE Foundation', 'Hamilton Families', 'Institute on Aging', 'Project Open Hand', 'San Francisco–Marin Food Bank']);
});
